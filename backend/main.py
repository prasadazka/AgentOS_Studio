...
WORKFLOW_UPLOADS_DIR = Path((os.getenv("AGENTOS_WORKFLOW_UPLOADS_DIR") or "").strip() or os.path.expanduser("~/.agent_os/workflow_uploads"))
WORKFLOW_OUTPUTS_DIR = Path((os.getenv("AGENTOS_WORKFLOW_OUTPUTS_DIR") or "").strip() or os.path.expanduser("~/.agent_os/workflow_outputs"))


def _resolve_workflow_output_dir(wid: str, rid: str, user_supplied: str = "") -> str:
    """Pick where a workflow run should write its outputs.

    Auto-route to the per-run GCS directory when:
      - the user left the field blank, OR
      - the value is a Windows-style local path AND we're running on a non-
        Windows host (i.e. the Cloud Run container) where that path can't exist.
    Otherwise honor the user-supplied path (local-dev convenience).
    """
    auto = WORKFLOW_OUTPUTS_DIR / wid / rid
    cleaned = (user_supplied or "").strip()
    if not cleaned:
        target = auto
    elif os.name != "nt" and re.match(r"^[A-Za-z]:[\\/]", cleaned):
        target = auto
    else:
        target = Path(cleaned)
...
def execute_workflow(wid: str, req: WorkflowExecuteRequest):
    ...
    # Auto-route output_path if the input_text is a JSON form and the user left
    # the field blank (or typed a local Windows path that won't exist here).
    input_text = req.input_text
    try:
        parsed = json.loads(input_text) if input_text else None
        if isinstance(parsed, dict) and "output_path" in parsed:
            parsed["output_path"] = _resolve_workflow_output_dir(
                wid, run["id"], parsed.get("output_path", "")
            )
            input_text = json.dumps(parsed)
    except (ValueError, TypeError):
        pass  # input_text wasn't JSON — leave it alone

    def stream():
        import traceback
        try:
            for event in workflow_manager.compile_and_execute(wf, input_text, run["id"]):
                yield f"data: {json.dumps(event)}\n\n"
        except Exception as e:
            traceback.print_exc()
            yield f"data: {json.dumps({'type': 'error', 'message': str(e)})}\n\n"
...
@app.get("/api/workflows/{wid}/runs/{rid}/files")
def list_workflow_run_files(wid: str, rid: str):
    """List output files generated by a specific workflow run."""
    run_dir = WORKFLOW_OUTPUTS_DIR / wid / rid
    if not run_dir.exists():
        return {"files": [], "total": 0, "output_dir": str(run_dir)}
    files = []
    for p in sorted(run_dir.rglob("*")):
        if p.is_file():
            ...
            files.append({
                "filename": p.name,
                "relpath": str(p.relative_to(run_dir)).replace("\\", "/"),
                "file_size": stat.st_size,
                "modified_at": datetime.fromtimestamp(stat.st_mtime, tz=timezone.utc).isoformat(),
            })
    return {"files": files, "total": len(files), "output_dir": str(run_dir)}


@app.get("/api/workflows/{wid}/runs/{rid}/files/download/{filename:path}")
def download_workflow_run_file(wid: str, rid: str, filename: str):
    """Stream a workflow-run-generated file with path-traversal protection."""
    run_dir = WORKFLOW_OUTPUTS_DIR / wid / rid
    if not run_dir.exists():
